...
def LSPIA_FUNC_cross_surface():
    ...
    p = 3  # degree
    q = 3
    P_h = int(row - 20)  # the number of control points
    P_l = int(col_even - 30)
    ...
    piece_u = 50
    piece_v = 100
    p_piece_u = np.linspace(param_u[0] + 1e-4, param_u[-1] - 1e-4, piece_u)
    p_piece_v = np.linspace(param_v[0] + 1e-4, param_v[-1] - 1e-4, piece_v)
    Nik_piece_u = np.zeros((piece_u, P_h))
    Nik_piece_v = np.zeros((piece_v, P_l))
    for i in range(piece_u):
        for j in range(P_h):
